...
metadata = read.csv('../data/sample-metadata.tsv', sep = '\t')
all.data = read.csv('../data/allSamples-readsdata.txt', sep = '\t')
...
ggsave(paste0(OUT_DIR_FIG, '/Male_FXS_methylation_prop.pdf'), width = 14, height = nrows * 3.6)
...
for (individual in individuals){
  ...
  ggsave(paste0(OUT_DIR_FIG, '/', individual,'_readsPropMeth.jpg'), width=7, height = nrows * 2.6)
}
...
for (individual in individuals) {
  ...
  ggsave(paste0(OUT_DIR_FIG, '/', individual,'_methvsallelebin.jpg'), width=7)
}
...
for (individual in individuals) {
  ...
  ggsave(paste0(OUT_DIR_FIG, '/', individual,'_methvsallelescatter.jpg'), width=7)
}

# Inidividual methylation allele bin scatter -----------------------------------

# Inidividual methylation allele bin histogram ---------------------------------
...
for (individual in individuals) {
  ...
  # Save plot
  ggsave(
    paste0(OUT_DIR_FIG, '/', individual,'_methvsallelebin.jpg'), 
    plot = p,
    width = 7
  )
}
...
ggplot(summary.data, aes(x = bin, y = total_reads, fill = meth_bin)) +
  ...
  theme_minimal()
ggsave(paste0(OUT_DIR_FIG, '/',  individual,'-reads-methbin.jpg'), width = 7, height = 4)
...
ggsave(paste0(OUT_DIR_FIG, '/percent-methbin-permutation-subset.jpg'), width = 5, height = 4)
...
ggplot(summary.data, aes(x = label, y = percentage, fill = meth_bin)) +
  ...
  labs (x = "Samples - Individual (Region)", y = "Percent reads (%)", fill = "Methylation", title = "Percentage of Methylated Premutation reads") +
  theme_minimal() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
ggsave(paste0(OUT_DIR_FIG, '/percent-methbin-permutation.jpg'), width = 6, height = 8)

# Premutation reads vs Methylation bin stack bar all samples -------------------

# Ridge plot of methylation distribution in premutation samples ----------------
...
# Allele length histogram colored by expansion range ---------------------------
...
for (individual in individuals){
  ...
  ggsave(paste0(OUT_DIR_FIG, '/', individual,'_test.jpg'), width=7)
  # }
}
...
# Combined plots for all samples -----------------------------------------------
...
for (individual in individuals){
  ...
  ggsave(paste0(OUT_DIR_FIG, '/', individual,'_FMR1_PureTarget_length-methylation.jpg'), width=7)
}
...
for (individual in unique(all.data$Individual)){
  ggplot(subset(all.data, Individual == individual), aes(x = allele_length, fill = prop_meth_level)) + 
    ...
    scale_fill_manual(values = c('blue4', 'blue3', 'blue2', 'blue1', 'royalblue1', 'snow2', 'tomato', 'red1', 'red2', 'red3', 'red4')) +
    labs(x = 'Allele length (motifs)', y = 'Reads', fill = 'Prop. Cs in repeat methylated') 
  ggsave(paste0(OUT_DIR_FIG, '/', individual,'_FMR1_PureTarget_methylation_prop.jpg'))
}
...
ggplot(summary.data, aes(x = Region, y = percent_above60, fill = bin)) +
  ...
  theme_minimal()
ggsave(paste0(OUT_DIR_FIG, '/FXPM5006-alleles-meth60.jpg'), width = 7, height = 4)
